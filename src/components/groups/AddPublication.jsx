...
export const AddPublication = ({
  setAddPublication,
  publication,
  setOpenPublicationId,
}) => {
  const { addNewPublication, updatePublication } = useGroupsStore();
  ...
  useEffect(() => {
    if (publication) {
      ...
      setValue('code', publication.code);
    }
  }, [publication]);

  const onSubmit = handleSubmit((data) => {
    data = { ...data, group: id, userId: user._id };
    ...
    if (!publication) {
      ...
      if (resp) {
        ...
        setTimeout(() => {
          socket.emit('nueva-publicacion', { userId: user._id, group: id });
        }, 5000);
      ...
      console.log('update');
      const updateRes = updatePublication(publication._id, formData);
      setOpenPublicationId(null);
      if (updateRes) {
        Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: 'Publicacion actulizada correctamente.',
          showConfirmButton: false,
          timer: 3000,
        });
      }
      ...
      >
        <div>
          ...
          <textarea
            ...
            className='rounded-lg bg-neutral-800 p-5 mt-4 w-full focus:outline-none focus:ring-0  '
